            ...
            <div className="row-span-6 flex items-center justify-center">
                ...
                {state.trialState.trialStarted && !state.trialState.trialEnded ?
                    <div className="flex flex-col items-center justify-center">
                        ...
                        <div className="flex">
                            {generateSolutionIndexOrder(4, state.currentTrialIndex.toString()).map((i: number, idx: number) => <img key={idx} onClick={() => { handleSolutionPressed(i) }} className={"mx-5 " + (state.trialState.feedbackStarted && i == 0 ? " border-8 border-lime-500" : "")} src={`/problem-solving-correlations-study/images/matrix-tasks/test/${state.currentTrialIndex}_${i + 1}.jpeg`}></img>)}
                        </div>
                    </div>
                    : <></>}
